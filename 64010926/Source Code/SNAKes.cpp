...
#define FILENAME "playrecord.txt"
...
int main()
{
	...
	gotoxy(25, 5);
	setcolor(7, 0);cout << "SNAKEes BY Supitchaya Prompalit 64010926";
	gotoxy(1, HEIGHT - 5);
	cout << "w -> up\ts ->  down\ta -> left\td -> right\tesc -> EXIT\tx -> EXIT";
	...
	do
	{
		...
		if (gameTime != 0 && gameTime % 15 == 0 && !poisonstate)
		{
			poisonstate = true;
			S.erasePoison();
			S.randPoison();
		}
		else if (gameTime % 15 != 0 && poisonstate) 
		{
			foodstate = false;
		}

		if (gameTime != 0 && gameTime % 15 == 0 && !blockstate)
		{
			blockstate = true;
			S.eraseBlock();
			S.randBlock();
	...
	setcolor(7, 0);cout << "! TOP 5 HIGH SCORES !";

	for (int i = 0;i < 5;i++)
	{
		string name = scoreData[i].name;
		int score = scoreData[i].score;
		
		gotoxy(WIDTH / 2 - 14, HEIGHT / 2 +5+ i);
		cout << "NAME : " << name;
		gotoxy(WIDTH / 2+5, HEIGHT / 2+5+ i);
		cout << "SCORE : " << score;
